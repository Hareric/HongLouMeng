...
chars = list(set(data))  # char vocabulary  构建单字set

data_size, _vocab_size = len(data), len(chars)
print 'data has %d characters, %d unique.' % (data_size, _vocab_size)
char_to_idx = {ch: i for i, ch in enumerate(chars)}  # 单字：index 字典
idx_to_char = {i: ch for i, ch in enumerate(chars)}  # index：单字 字典
...
context_of_idx = [char_to_idx[ch] for ch in data]  # 将原文本数据转化为vector
...
def data_iterator(raw_data, batch_size, num_steps):
    ...
    for i in range(batch_size):
        # data的shape是(batch_size, batch_len)，每一行是连贯的一段，一次可输入多个段
